		...
		<div
			...
			<h1 v-else class="w-full flex justify-between items-center">
				...
				<DatePicker dark :color="'purple'" is-dark v-model="date" mode="date">
					<template #default="{ togglePopover }">
						<span @click="togglePopover"
							class="text-md font-bold px-2 py-1 dark:hover:bg-neutral-300 hover:text-accent hover:bg-neutral-900 rounded-xl transition-all duration-200 ease-in-out cursor-pointer text-center">{{
								$formatDateWords(date) }}</span>
					</template>
				</DatePicker>

				<span class="font-bold text-md">GHC {{ today_daily_sales }}.00</span>
			</h1>
...
const { total_apls, is_mobile: ISM } = storeToRefs(useAppStore())
