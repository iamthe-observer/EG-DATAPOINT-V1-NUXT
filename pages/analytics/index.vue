<template>
	<div class="w-full h-full">
		...
		<div v-if="role && !loading" class="w-full h-full rounded-2xl overflow-y-auto" id="style-2">
			<div class="w-full min-h-full rounded-s-2xl bg-neutral-800 dark:bg-neutral-50 dark:shadow-xl col-span-full row-span-full p-3 overflow-y-auto text-justify relative flex flex-col gap-5"
							...
							<div class="flex w-full">
								<div :class="[
									...
									<span v-if="transaction.type == 'entry' || transaction.type == 'discount'"
										...
										{{ get_transac_price(transaction) }}.00 </span>
				...
				<section class="grid grid-cols-4 w-full gap-5 mb-3">
					<div
						...
						<LineChart :class="[ISM ? 'h-fit' : 'h-[500px]']" :chartData="lineData"
							:options="lineOptions" />
					</div>
				</section>
...
function get_transac_price(tr: Transactions) {
	const created_at = total_apls.value.find(p => p.apl_id == tr.apl_id)?.created_at;
	if (created_at !== undefined && currDate.value.setHours(0, 0, 0) == new Date(created_at).setHours(0, 0, 0)) {
		if (tr.type == 'delete') {
			return 0
		}
		else if (tr.type == 'discount' || tr.type == 'entry') {
			return tr.balance_before
		}
