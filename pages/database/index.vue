<template>
	<div class="w-full relative h-full rounded-2xl pb-2">
		<div id="style-1" :class="[
			'bg-white max-w-full w-full h-full max-h-full rounded-2xl bg-neutral-80 col-span-full row-span-full scrollbar text-justify p-2 relative flex flex-col',
			...
			<div class="w-full h-full rounded-xl flex gap-2 overflow-hidden pt-2">
				...
				<div @click="handlePage('tasks')" :class="[
					'transition-all duration-500 ease-in-out bg-neutral-900 group rounded-xl relative',
					curr_page !== 'tasks' ? 'flex-1' : 'w-4/5'
				]">
				...
				<div @click="handlePage('requests')" :class="[
					'transition-all duration-500 ease-in-out bg-neutral-900 group rounded-xl relative',
					curr_page !== 'requests' ? 'flex-1' : 'w-4/5'
				]">
				...
				<div @click="handlePage('announcements')" :class="[
					'transition-all duration-500 ease-in-out bg-neutral-900 group rounded-xl relative',
					curr_page !== 'announcements' ? 'flex-1' : 'w-4/5'
				]">
