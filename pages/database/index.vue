				...
				<div id="style-1" :class="[
					...
					<div ref="header" :class="[
						...
						<div @click="useSearchStore().setIfSearch(true)" v-if="!if_search" ref="search_bar"
							class="z-[200] w-full flex justify-end search_bar dark:text-warning bg-neutral-900/70 dark:bg-neutral-50/30 outline outline-4 outline-neutral-900/30 dark:outline-neutral-50/60 mb-4 backdrop-blur rounded-xl shadow-xl transition-all duration-300 ease-out dark:outline-none hover:scale-95">
					...
					<div class="w-full h-full rounded-xl flex gap-2 overflow-hidden pt-2">
						...
						<div @click="handlePage('tasks')" :class="[
							'transition-all duration-500 ease-in-out dark:bg-neutral-50/30 bg-neutral-900/40 border-4 border-neutral-900/30 dark:border-neutral-50/60 mb-4 backdrop-blur-md group rounded-xl relative',
							curr_page !== 'tasks' ? 'flex-1' : 'w-4/5 dark:shadow-xl dark:bg-neutral-50 bg-neutral-800'
						]">
							<span v-if="curr_page != 'tasks' && curr_page != ''"
								class="vertical-text absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 tracking-[-0.2rem] font-semibold text-neutral-50 dark:group-hover:text-warning group-hover:text-secondary text-[2rem] cursor-pointer transition-all duration-300 ease-out group-hover:scale-110">TASKS</span>

							<TasksTab :curr_page="curr_page" />
						</div>

						<div @click="handlePage('requests')" :class="[
							'transition-all duration-500 ease-in-out dark:bg-neutral-50/30 bg-neutral-900/40 border-4 border-neutral-900/30 dark:border-neutral-50/60 mb-4 backdrop-blur-md rounded-xl relative',
							...
							<span v-if="curr_page != 'requests' && curr_page != ''"
								class="vertical-text absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 tracking-[-0.2rem] font-semibold text-neutral-50 dark:group-hover:text-warning group-hover:text-secondary text-[2rem] cursor-pointer transition-all duration-300 ease-out group-hover:scale-110">REQUESTS</span>
						...
						<div @click="handlePage('announcements')" :class="[
							...
							<span v-if="curr_page != 'announcements' && curr_page != ''"
								class="vertical-text absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 tracking-[-0.2rem] font-semibold text-neutral-50 dark:group-hover:text-warning group-hover:text-secondary text-[2rem] cursor-pointer transition-all duration-300 ease-out group-hover:scale-110">ANNOUNCEMENTS</span>
