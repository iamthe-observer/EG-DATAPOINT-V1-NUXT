<template>
	<div ref="container" class="bg-white w-full h-full flex flex-col text-black absolute inset-0">
		<div class="w-full h-full flex flex-col relative">
			...
			<h1 ref="header_ref"
				...
				<span ref="discount_ref"
					...
					<p class="text-lg flex flex-col">
						<span id="text" class="text-2xl font-bold">Add a new Applicant</span>
						<button v-if="if_hover_discount" onclick="request_modal.showModal()"
							class="request_btn btn bg-white border-2 text-black group-hover:bg-secondary border-black btn-xs dark:group-hover:text-black rounded-none group-hover:shadow-neo-box">Request
							Discount</button>
					</p>

					<dialog id="request_modal" class="modal">
						<form method="dialog" class="modal-box bg-white rounded-none border-2 border-black flex flex-col gap-4">
				...
				<div class="h-fit justify-center items-center text-md flex gap-2">
					...
					}" class="btn btn-ghost hover:border-2 hover:border-black text-black group hover:bg-gold-600 group font-bold rounded-none group hover:shadow-neo-box mr-3">
						<span class="translate-y-[1px]">Reset</span>
						<svg xmlns="http://www.w3.org/2000/svg" class="w-4 aspect-square stroke-red-500 group-hover:stroke-black"
							viewBox="0 0 24 24">
					...
					}" class="btn border-2 border-black rounded-none bg-gold-600 text-black font-bold group hover:bg-green-500 group hover:shadow-neo-box hover:-translate-y-2">
						<span class="translate-y-[1px]">SUBMIT</span>
						<SvgsCedis
							class="w-4 aspect-square stroke-black fill-black transition-all duration-150 ease-linear group-hover:fill-black translate-y-[1px]" />
						<span class="translate-y-[1px]">{{ price }}.00</span>
					</button>
			...
			<main ref="container" class="flex-1 rounded-xl overflow-y-auto py-2 pb-6 flex flex-col pl-12" id="style-1">

				<p class="font-bold uppercase text-3xl mx-auto pt-10 pb-5">Primary Applicant</p>
				<FieldSetPrimeApl />

				<p v-if="applicant.pmarital_status == 'MARRIED'" class="font-bold uppercase text-3xl mx-auto pt-14 pb-5">Secondary
					Applicant</p>
				<FieldSetSecApl v-if="applicant.pmarital_status == 'MARRIED'" />

				<p v-if="applicant.children_number > 0" class="font-bold uppercase text-3xl mx-auto pt-14 pb-5">
					Wards
				</p>
