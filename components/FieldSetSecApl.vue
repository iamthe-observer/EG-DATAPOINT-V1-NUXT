<template>
	<div class="__newApl relative w-full grid grid-cols-12 gap-2 px-5 py-2">
		...
		<div class="flex gap-4 col-span-10 pl-6 justify-center">
			<TextInput v-model="applicant.slastName">Last Name</TextInput>
			<TextInput v-model="applicant.sfirstName">First Name</TextInput>
		...
		<div class="flex gap-4 col-span-10 pl-6 justify-center">
			<DatePicker dark :color="'purple'" is-dark v-model="applicant.sdob" mode="date">
				<template #default="{ togglePopover }">
					<TextInput :icon="true" :value="applicant.sdob ? $formatDate(new Date(applicant.sdob!)) : ''"
						@click="togglePopover">
						Date of Birth
					</TextInput>
				</template>
			</DatePicker>
			<SelectInput :options="['MALE', 'FEMALE']" v-model="applicant.sgender">Gender
			</SelectInput>
			<TextInput v-model="applicant.scity_ob">City of Birth</TextInput>
		</div>

		<div class="flex gap-4 col-span-12 justify-center">
			<SelectInput :options="$countries" v-model="applicant.scountry_ob">Country of
				Birth
			</SelectInput>
			<TextInput v-model="applicant.scontact">Phone Number</TextInput>
		</div>
	</div>
</template>
