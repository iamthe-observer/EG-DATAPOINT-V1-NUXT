<template>
	<div class="col-span-3 row-span-15 px-2 flex flex-col items-center gap-5">
		<!-- registered apls -->
		<div
			class="backdrop-blur dark:bg-white/30 bg-neutral-900/40 rounded-xl outline outline-4 outline-neutral-100/20 hover:outline-neutral-100/50 transition-all duration-200 ease-in-out w-full h-fit drop-shadow-lg flex flex-col items-center justify-between gap-2 px-3 py-1 relative">
			<h2 class="text-2xl flex justify-between w-full sticky top-0">
				<span class="font-bold text-neutral-50 drop-shadow-lg">Registered Apls</span>
				<span v-if="!role" class="font-bold text-neutral-50 drop-shadow-lg">{{ registered_apls.length }}/{{
					all_my_apls.length
				}}</span>
				<span v-else class="font-bold">{{ admin_registered_apls.length }}<span
		...
		<div class="backdrop-blur dark:bg-white/30 bg-neutral-900/40 rounded-xl outline outline-4 outline-neutral-100/20 hover:outline-neutral-100/50 transition-all duration-200 ease-in-out w-full h-full flex flex-col items-center gap-4 px-3 py-3 relative overflow-y-auto"
			id="style-2">
			<h2
				class="text-2xl flex justify-between w-full sticky top-0 backdrop-blur-xl dark:bg-neutral-900/50 bg-neutral-900/75 rounded-xl px-4 py-1 drop-shadow-lg">
				<span class="font-bold text-white">Incomplete Apls Pics</span>
				<span class="font-bold text-white">{{ incomplete_apl_pics.length }}</span>
			</h2>

			<div v-if="incomplete_apl_pics.length"
				class="text-white font-bold w-full flex gap-2 items-center group cursor-pointer"
			...
			<div v-else
				class="w-full h-full flex flex-col items-center justify-center text-center font-bold text-neutral-50 drop-shadow-lg">
