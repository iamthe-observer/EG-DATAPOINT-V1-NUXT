<template>
	<div @click="() => { $router.push(`/applicant/${result.apl_id}`); useViewAplStore().setID(result.apl_id!) }"
		class="card card-side bg-black/50 dark:bg-neutral-200/20 shadow-xl w-[60%] cursor-pointer hover:bg-white/10 dark:hover:bg-neutral-50/50 dark:hover:text-black dark:text-white transition-all group duration-400 ease-in-out">
		...
		<div class="card-body p-2 dark:group-hover:text-black" @click="$emit('recentSearch', rSearchParams)">
			...
			<div
				class="card-actions flex flex-col gap-1 items-end justify-end text-yellow-300 group-hover:text-white dark:group-hover:text-black text-xs">
				<button class="dark:drop-shadow-lg">{{ $formatDateTime(new
					Date(result.created_at!))
				}}</button>
				<button class="dark:drop-shadow-lg">{{ $formatDateWords(new
					Date(result.created_at!))
				}}</button>
			</div>
		</div>
