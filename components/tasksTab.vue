<template>
	<div v-if="curr_page == 'tasks' || curr_page == ''"
		...
		<h1 class="py-1 px-1 text-xl font-semibold flex justify-between">
			<span
				:class="['transition-all duration-300 ease-out mb-5 text-white', curr_page == 'tasks' ? 'text-4xl' : '']">Tasks</span>
			...
			<span class="text-white">{{ curr_filtered_tasks.length }}</span>
		</h1>
		...
		<div v-if="curr_filtered_tasks.length == 0" class="w-full h-full grid place-items-center">
			<div class="flex flex-col items-center gap-2">
				...
				<span class="text-center text-white">No Tasks</span>
			</div>
		</div>

		<div v-else id="style-1"
			class="bg-neutral-900/40 dark:bg-neutral-100/30 text-white flex flex-col gap-3 rounded-xl overflow-y-auto">
				...
				<span class="text-neutral-600 w-10 text-xs grid place-items-center">
					<div class="form-control">
						<label class="label cursor-pointer">
							<input @click="useTasksStore().updateTask(task.done!, task.id!)" type="checkbox"
								:value="task.done" class="checkbox checkbox-primary" />
						</label>
					</div>
				</span>
				<span
					class="flex flex-col justify-center truncate truncate- w-[100px] flex-1 overflow-x-hidden text-sm">
					<span :class="['truncate-', task.done ? 'text-neutral-600 line-through' : '']">{{ task.body
					}}</span>
				</span>
