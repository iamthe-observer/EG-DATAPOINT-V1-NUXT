<template>
	<div class="w-full h-full col-span-6 row-span-7 p-2">
		...
		<div v-if="!role" class="stats shadow w-full h-full overflow-x-hidden">
			<div class="stat">
				...
				<div class="flex w-full stat-desc"><span class="">{{ perc_compared_to_yesterday }}</span>
				</div>
			</div>
		</div>

		<!-- admin -->
		<div v-if="role" v-dragscroll id="style-1"
			class="stats shadow w-full h-full overflow-x-hidden hover:cursor-grab active:cursor-grabbing">
			<div class="stat">
				<div class="stat-figure text-secondary">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
						class="inline-block w-8 h-8 stroke-current">
