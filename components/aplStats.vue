<template>
	<div class="w-full h-full col-span-6 row-span-7 p-2">
		...
		<div v-if="!role"
			class="stats backdrop-blur dark:bg-white/30 bg-neutral-900/40 rounded-xl outline outline-4 outline-neutral-100/20 hover:outline-neutral-100/50 transition-all duration-200 ease-in-out w-full h-full overflow-x-hidden dark">
			<div class="stat">
				<div :class="['stat-figure', dark_mode ? 'text-yellow-300' : 'text-yellow-300']">
				...
				<div
					:class="['stat-title font-bold text-xl drop-shadow-lg', dark_mode ? 'text-neutral-50' : 'text-neutral-50']">
					Today's
					Sales
				</div>
				<div
					:class="[dark_mode ? 'text-neutral-900' : 'text-neutral-400', 'stat-value flex items-center gap-1']">
					...
					<span @dblclick="showPrice" v-else class="flex">
						<svg v-for="i in 4" xmlns="http://www.w3.org/2000/svg" width="32" height="32"
							viewBox="0 0 24 24" class="drop-shadow-lg">
							<path fill="white"
				...
				<div class="flex w-full text-sm stat-desc drop-shadow-lg">
					<span :class="[dark_mode ? 'text-yellow-300' : 'text-purple-600', 'whitespace-pre-wrap']">{{
